  ...
  return (
    <Card className="col-span-3">
      <CardHeader className="pb-2">
        <CardTitle className="text-2xl font-bold tracking-tight">SPY Overview</CardTitle>
      </CardHeader>
      <CardContent>
        {spyData && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">${spyData.price.toFixed(2)}</p>
            </div>
            
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Change</p>
              <div className="flex items-center gap-1">
                {isPositive ? <ArrowUp className="h-4 w-4 text-positive" /> : <ArrowDown className="h-4 w-4 text-negative" />}
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">{(spyData.volume / 1000000).toFixed(1)}M</p>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-2xl font-bold">{spyData.vix.toFixed(2)}</p>
              </div>
            </div>
            
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Day Range</p>
              <p className="text-lg font-medium">${spyData.low.toFixed(2)} - ${spyData.high.toFixed(2)}</p>
            </div>
            
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Open</p>
              <p className="text-lg font-medium">${spyData.open.toFixed(2)}</p>
            </div>
            
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Previous Close</p>
              <p className="text-lg font-medium">${spyData.previousClose.toFixed(2)}</p>
            </div>
