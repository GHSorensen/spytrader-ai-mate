...
export const useMockData = () => {
  ...
  const mockPerformanceSummary: PerformanceSummary = useMemo(() => ({
    today: {
      profit: 1250.75,
  ...
  const mockTradeAnalytics: TradeAnalytics = useMemo(() => ({
    byStrategy: [
      {
        strategyId: '1',
  ...
  const generateEquityCurveData = useMemo(() => (days: number, startValue: number = 100000) => {
    const data = [];
  ...
  const generateMonthlyPerformanceData = useMemo(() => (months: number) => {
    const data = [];
  ...
  const generateProfitDistributionData = useMemo(() => () => {
    const ranges = [
      '-50% to -40%',
      '-40% to -30%',
      '-30% to -20%',
  ...
  const generateDrawdownsData = useMemo(() => (count: number) => {
    const data = [];
    const currentDate = new Date();
  ...
  const mockPerformanceChartData: PerformanceChartData = useMemo(() => ({
  ...
  }), [generateEquityCurveData, generateProfitDistributionData, generateMonthlyPerformanceData, generateDrawdownsData]);

  // Mock backtest results
  const mockBacktestResults: BacktestResult[] = useMemo(() => [
    {
      strategyId: '1',
